...
export default function Settings() {
  ...
  const Btn = ({ children, onClick, color, disabled }) => (
    <button onClick={onClick} disabled={disabled} style={{ padding:'7px 14px', border:'none', borderRadius:7, fontWeight:600, fontSize:13, background:color||'var(--surface2)', color:color?'#fff':'var(--text2)', opacity:disabled?.6:1, transition:'opacity .15s' }}>{children}</button>
  );
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:18, maxWidth:860 }}>
            ...
            return (
              <div key={p} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--surface2)', borderRadius:8, padding:'12px 14px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <span style={{ padding:'3px 8px', borderRadius:5, fontSize:11, fontWeight:600, background:s.enabled?'rgba(16,185,129,.15)':'rgba(239,68,68,.15)', color:s.enabled?'var(--ok)':'var(--danger)' }}>{s.enabled?'● Активен':'○ Выкл'}</span>
      ...
      <div style={{ display:'flex', gap:3, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:9, padding:3, width:'fit-content' }}>
        {[['costs','💰 Себестоимость'],['log','📋 Лог сборов']].map(([id,label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ padding:'7px 16px', border:'none', borderRadius:7, fontSize:13, fontWeight:500, background:tab===id?'var(--surface2)':'transparent', color:tab===id?'var(--text)':'var(--text2)' }}>{label}</button>
        ))}
      </div>
      ...
      {tab==='costs' && (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:18 }}>
          ...
          {costs.length===0 ? (
            <div style={{ padding:24, textAlign:'center', color:'var(--text2)' }}>Добавь себестоимость — появится расчёт маржи в Аналитике.</div>
          ) : (
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead><tr>{['Площадка','Артикул','Название','Себестоимость',''].map(h => <th key={h} style={{ padding:'8px 12px', textAlign:'left', color:'var(--text2)', fontWeight:500, fontSize:12, borderBottom:'1px solid var(--border)' }}>{h}</th>)}</tr></thead>
              <tbody>
                {costs.map(c => (
                  <tr key={c.id} style={{ borderBottom:'1px solid var(--border)' }}>
                    ...
                    <td style={{ padding:'9px 12px' }}><button onClick={async () => { await deleteCost(c.id); setCosts(costs.filter(x=>x.id!==c.id)); }} style={{ border:'none', background:'transparent', color:'var(--danger)', fontSize:18, cursor:'pointer' }}>×</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab==='log' && (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius)', overflow:'hidden' }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            <thead><tr>{['Время','Площадка','Тип','Статус','Записей','Ошибка'].map(h => <th key={h} style={{ padding:'8px 12px', textAlign:'left', color:'var(--text2)', fontWeight:500, borderBottom:'1px solid var(--border)' }}>{h}</th>)}</tr></thead>
            <tbody>
              ...
              {log.map(l => (
                <tr key={l.id} style={{ borderBottom:'1px solid var(--border)' }}>
                  ...
                  <td style={{ padding:'8px 12px', color:l.status==='success'?'var(--ok)':l.status==='error'?'var(--danger)':'var(--warn)' }}>{l.status==='success'?'✅':l.status==='error'?'❌':'🔄'} {l.status}</td>
