  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <input placeholder="Поиск по артикулу или названию..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: 280 }} />
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        {loading ? <div style={{ padding: 32, textAlign: 'center', color: 'var(--text2)' }}>Загрузка...</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>
                {['Артикул','Наименование', activeTab==='wb' ? 'Остаток' : 'FBO', activeTab==='ozon' ? 'FBS' : null, 'Итого','Статус'].filter(Boolean).map(h =>
                ...
                return (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    {activeTab === 'wb'
                      ...
                      : <>
                          <td style={{ padding: '9px 12px' }}>{p.fbo_qty||0}</td>
                          <td style={{ padding: '9px 12px' }}>{p.fbs_qty||0}</td>
                        </>
                    }
                    {activeTab === 'ozon' && <td style={{ padding: '9px 12px', fontWeight: 700, color: b.color }}>{qty}</td>}
                    <td style={{ padding: '9px 12px' }}>
                      <span style={{ padding: '2px 8px', borderRadius: 5, fontSize: 11, fontWeight: 600, background: b.bg, color: b.color }}>{b.label}</span>
                    </td>
                  </tr>
                );
              })}
