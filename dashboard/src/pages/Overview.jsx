...
export default function Overview({ platform, dateFrom, dateTo }) {
  ...
  const G = ({ cols, children }) => <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${cols||170}px, 1fr))`, gap: 10 }}>{children}</div>;
      ...
      {(platform === 'all' || platform === 'wb') && (
        <div>
          ...
          <G>
            ...
            <MetricCard label="ДРР" value={Number(wb.drr||0)} suffix="%"
              color={Number(wb.drr) > 20 ? 'var(--danger)' : 'var(--ok)'}
              sub={Number(wb.drr) > 20 ? '⚠️ Выше нормы' : '✅ В норме'} />
          </G>
        </div>
      )}
      ...
      {(platform === 'all' || platform === 'ozon') && (
        <div>
          ...
          <G>
            ...
            <MetricCard label="ДРР" value={Number(oz.drr||0)} suffix="%"
              color={Number(oz.drr) > 20 ? 'var(--danger)' : 'var(--ok)'}
              sub={Number(oz.drr) > 20 ? '⚠️ Выше нормы' : '✅ В норме'} />
          </G>
        </div>
      )}
