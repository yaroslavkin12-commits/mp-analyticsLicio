...
async function collectStocks() {
    ...
    for (const r of rows) {
      await query(
        `INSERT INTO ozon_stocks
          (snapshot_date, sku, offer_id, product_name, fbo_present, fbo_reserved,
           fbs_present, warehouse_id, warehouse_name)
         VALUES (?,?,?,?,?,?,?,?,?)`,
        [
          today, r.sku, r.item_code, r.item_name,
          r.free_to_sell_amount || 0, r.reserved_amount || 0,
          r.promised_amount || 0,
          r.warehouse_id || null, r.warehouse_name || null,
        ]
      );
      total++;
    }
  ...
  console.log(`[Ozon] Остатки сохранено: ${total}`);
  return total;
}
