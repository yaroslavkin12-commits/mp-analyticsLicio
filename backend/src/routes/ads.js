...
router.get('/discounts/summary', async (req, res) => {
  try {
    const cabinet = req.query.cabinet || 'licio';
...
router.get('/discounts/feed', async (req, res) => {
  try {
    const cabinet = req.query.cabinet || 'licio';
    const days = Math.max(1, Math.min(90, parseInt(req.query.days, 10) || 30));
...
router.get('/discounts/settings', async (req, res) => {
  try {
    const cabinet = req.query.cabinet || 'licio';
...
router.post('/discounts/settings', async (req, res) => {
  try {
