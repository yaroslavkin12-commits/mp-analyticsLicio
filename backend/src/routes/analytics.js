...
function extractCategory(productName) {
  ...
  if (n.includes('детск') || n.includes('child'))
    return n.includes('футболк') ? 'Футболка детская' : 'Детское';
  if (n.includes('худи'))
    return n.includes('женск') || n.includes(' w ') || n.includes('w,') ? 'Худи женское' : 'Худи мужское';
  ...
  if (n.includes('брюк') || n.includes('штан'))
    return n.includes('женск') || n.includes(' w ') || n.includes('w,') ? 'Брюки женские' : 'Брюки мужские';
  if (n.includes('шорт'))
    return n.includes('женск') || n.includes(' w ') || n.includes('w,') ? 'Шорты женские' : 'Шорты мужские';
...
router.get('/ozon/categories', async (req, res) => {
  try {
    ...
    const products = await query(`
      ...
      FULL OUTER JOIN (
        SELECT DISTINCT sku, offer_id, product_name FROM ozon_orders
        WHERE date BETWEEN $1 AND $2
      ) o ON o.sku = a.sku
      WHERE a.date BETWEEN $1 AND $2 OR o.offer_id IS NOT NULL
      ORDER BY offer_id
    `, [from, to]);
    ...
    for (const p of products) {
      ...
      const color = parts.length >= 3 ? parts[2].trim().split(' ').slice(0,3).join(' ') : 'Без цвета';
      if (!tree[cat][color]) tree[cat][color] = [];
      tree[cat][color].push({ offer_id: p.offer_id, product_name: p.product_name });
    }
...
router.get('/ozon', async (req, res) => {
  try {
    ...
    if (offerIds) {
      ...
      if (ids.length) {
        const phs = ids.map((_,i) => `$${i+3}`).join(',');
        offerFilter = `AND COALESCE(a.offer_id, o.offer_id) IN (${phs})`;
        extraParams = ids;
      }
    }
    ...
    if (groupBy === 'day') {
      const rows = await query(`
        SELECT
          ...
          CASE WHEN COALESCE(a.hits_view,0)>0
            THEN ROUND(COALESCE(o_all.orders_qty,0)::numeric/a.hits_view*100,2) ELSE 0 END as cr_order_pct,
          CASE WHEN COALESCE(o_all.orders_qty,0)>0
            THEN ROUND(COALESCE(o_del.sales_qty,0)::numeric/o_all.orders_qty*100,1) ELSE 0 END as redemption_pct
        ...
        LEFT JOIN (
          SELECT date, SUM(price*quantity) as orders_sum, SUM(quantity) as orders_qty
        ...
        WHERE COALESCE(a.hits_view,0)+COALESCE(o_all.orders_qty,0) > 0
        ORDER BY d.date
      `, [from, to]);
    ...
    const rows = await query(`
      SELECT
        COALESCE(a.sku::text, o_all.sku::text)       as sku,
        COALESCE(a.offer_id, o_all.offer_id)         as offer_id,
        COALESCE(a.product_name, o_all.product_name) as product_name,
        COALESCE(a.hits_view, 0)      as hits_view,
        COALESCE(a.hits_view_pdp, 0)  as hits_view_pdp,
        COALESCE(a.hits_tocart, 0)    as hits_tocart,
        -- Заказы = ВСЕ
        ...
        CASE WHEN COALESCE(a.hits_tocart,0)>0
          THEN ROUND(COALESCE(o_all.orders_qty,0)::numeric/a.hits_tocart*100,1) ELSE 0 END as cr_cart_to_order_pct,
        CASE WHEN COALESCE(o_all.orders_qty,0)>0
          THEN ROUND(COALESCE(o_del.sales_qty,0)::numeric/o_all.orders_qty*100,1) ELSE 0 END as redemption_pct,
        -- Реклама
        COALESCE(ads.spend, 0) as ad_spend,
        CASE WHEN COALESCE(o_all.orders_sum,0)>0
          THEN ROUND(COALESCE(ads.spend,0)/o_all.orders_sum*100,1) ELSE 0 END as drr,
        COALESCE(pc.cost_price, 0) as cost_price,
        CASE WHEN COALESCE(o_all.orders_qty,0)>0
          THEN ROUND(COALESCE(o_all.orders_sum,0)/o_all.orders_qty,0) ELSE 0 END as avg_price,
        (SELECT SUM(fbo_present+fbs_present) FROM ozon_stocks
         WHERE sku=COALESCE(a.sku,o_all.sku)
      ...
      FROM (
        SELECT sku, offer_id, product_name,
          ...
          SUM(hits_tocart) as hits_tocart, SUM(cancellations) as cancellations
        FROM ozon_analytics WHERE date BETWEEN $1 AND $2
      ...
      LEFT JOIN (
        SELECT sku, COUNT(*) as returns_qty
        FROM ozon_orders WHERE date BETWEEN $1 AND $2 AND status LIKE '%return%'
        GROUP BY sku
      ) o_ret ON o_ret.sku = COALESCE(a.sku, o_all.sku)
      LEFT JOIN (
        SELECT sku, SUM(spend) as spend FROM ozon_ads
        WHERE date BETWEEN $1 AND $2 GROUP BY sku
      ) ads ON ads.sku = COALESCE(a.sku, o_all.sku)
      LEFT JOIN product_costs pc
        ON pc.platform='ozon' AND pc.article = COALESCE(a.offer_id, o_all.offer_id)
      ORDER BY COALESCE(o_all.orders_sum, 0) DESC LIMIT 500
    `, [from, to, ...extraParams]);
    ...
    const [totals] = await query(`
      SELECT
        ...
        SUM(o_all.orders_qty) as orders_qty,
        SUM(o_all.orders_sum) as orders_sum,
        SUM(o_del.sales_qty)  as delivered_qty,
        SUM(o_del.revenue)    as revenue,
        CASE WHEN SUM(a.hits_view)>0
          THEN ROUND(SUM(a.hits_view_pdp)::numeric/SUM(a.hits_view)*100,2) ELSE 0 END as ctr_pct,
        CASE WHEN SUM(a.hits_view)>0
          THEN ROUND(SUM(o_all.orders_qty)::numeric/SUM(a.hits_view)*100,2) ELSE 0 END as cr_order_pct,
        CASE WHEN SUM(o_all.orders_qty)>0
          THEN ROUND(SUM(o_del.sales_qty)::numeric/SUM(o_all.orders_qty)*100,1) ELSE 0 END as redemption_pct
      FROM (
  ...
  try {
    ...
    const [r] = await query(`
      SELECT
        ...
        SUM(o_all.orders_qty)  as orders,
        SUM(o_del.sales_qty)   as delivered,
        SUM(o_all.orders_sum)  as orders_sum,
        SUM(o_del.revenue)     as revenue,
        CASE WHEN SUM(a.hits_view)>0
        ...
        CASE WHEN SUM(a.hits_tocart)>0
          THEN ROUND(SUM(o_all.orders_qty)::numeric/SUM(a.hits_tocart)*100,1) ELSE 0 END as cart_to_order,
        CASE WHEN SUM(o_all.orders_qty)>0
          THEN ROUND(SUM(o_del.sales_qty)::numeric/SUM(o_all.orders_qty)*100,1) ELSE 0 END as order_to_delivered,
        CASE WHEN SUM(a.hits_view)>0
          THEN ROUND(SUM(o_all.orders_qty)::numeric/SUM(a.hits_view)*100,2) ELSE 0 END as overall_cr
      FROM (
        SELECT sku, SUM(hits_view) as hits_view, SUM(hits_view_pdp) as hits_view_pdp, SUM(hits_tocart) as hits_tocart
        FROM ozon_analytics WHERE date BETWEEN $1 AND $2 GROUP BY sku
      ) a
